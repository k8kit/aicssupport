class CityMayorApplicants {
  ...
  setupEventListeners() {
    ...
    const signSelectedBtn = document.getElementById('sign-selected-btn')
    if (signSelectedBtn) {
        signSelectedBtn.addEventListener('click', () => this.batchSignApplications())
    }
    ...
    if (selectAllCheckbox) {
        selectAllCheckbox.addEventListener('change', (e) => {
            ...
            checkboxes.forEach(cb => {
                ...
                this.toggleApplicationSelection(cb.value, e.target.checked)
            })
        })
    }
  }
  ...
  async loadApplicants() {
    try {
      ...
      const response = await this.adminAuth.makeAuthenticatedRequest("../api/citymayor/applicants.php?" + queryString)
      if (!response) return

      const result = await response.json()

      if (result.success) {
        this.allApplicants = result.data // ✅ store for pagination
        this.renderApplicantsTable()
        this.renderPaginationControls()
      }
    } catch (error) {
      ...
      <tr>
        ${this.batchMode ? `
          <td class="batch-select-column">
            <input type="checkbox" 
                  class="application-checkbox form-check-input" 
                  value="${applicant.id}"
                  ${this.selectedApplications.has(applicant.id) ? 'checked' : ''}
                  onchange="cityMayorApplicants.toggleApplicationSelection('${applicant.id}', this.checked)">
          </td>
        ` : ''}
        <td>${this.formatDate(applicant.updated_at)}</td>
        <td><span class="fw-bold text-primary">${applicant.reference_no}</span></td>
        ...
        <td>
          <div class="btn-group" role="group">
            <button class="btn btn-sm btn-outline-primary action-btn" 
              onclick="cityMayorApplicants.viewApplication('${applicant.id}')"
              title="View Application">
              <i class="bi bi-file-text me-1"></i>
            </button>
            <button class="btn btn-sm btn-outline-success action-btn ms-1" 
              onclick="cityMayorApplicants.updateApplicationStatus('${applicant.id}', 'ready')"
              title="Forward to Release">
  ...
  renderPaginationControls() {
    const totalItems = (this.allApplicants || []).filter(a =>
      !this.searchQuery ||
      a.reference_no.toLowerCase().includes(this.searchQuery) ||
      a.client_full_name.toLowerCase().includes(this.searchQuery)
    ).length
                            ...
                            <div class="form-group col-6">
                                <div class="signature-section">
                                    ...
                                    <div class="text-center mt-2">
                                        <button type="button" class="btn btn-sm btn-outline-primary" id="add-signature">
  ...
  setupModalEventListeners() {
    const rejectBtn = document.getElementById("reject-application")
    const forwardBtn = document.getElementById("forward-application")
    ...
    if (rejectBtn) {
      ...
      document.getElementById("reject-application").addEventListener("click", () => {
        this.updateApplicationStatus(this.currentApplicationId, "rejected")
      })
    }
    ...
    if (forwardBtn) {
      ...
      document.getElementById("forward-application").addEventListener("click", () => {
        this.updateApplicationStatus(this.currentApplicationId, "waiting for release")
      })
    }

    if (addSignatureBtn) {
      addSignatureBtn.replaceWith(addSignatureBtn.cloneNode(true))
      document.getElementById("add-signature").addEventListener("click", () => {
        this.showSavedSignatureModal()
      })
    }
  }
  ...
  renderFamilyRows(familyMembers) {
    ...
    try {
      const response = await this.adminAuth.makeAuthenticatedRequest("../api/admin/get-signature.php");
      if (!response) return null;

      const result = await response.json();
      if (result.success && result.data?.signature_path) {
        return result.data;
      }
      return null;
    } catch (error) {
      console.error("Error checking existing mayor signature:", error);
      return null;
    }
  }

  // ✅ Show Saved Signature Modal (Mayor version)
  async showSavedSignatureModal() {
    try {
      ...
      const result = await response.json();
      const container = document.getElementById("saved-signature-content");
      if (!container) return;

      const modal = new bootstrap.Modal(document.getElementById("viewSavedSignatureModal"));

      // --- CASE 1: Mayor already has a saved signature ---
      if (result.success && result.data?.signature_path) {
        container.innerHTML = `
          <img src="../${result.data.signature_path}" alt="Saved Signature" class="img-fluid border rounded mb-2">
          <p class="mb-0 fw-bold">${result.data.full_name}</p>
        `;
      } else {
        // --- CASE 2: No signature — allow upload ---
        container.innerHTML = `
          <div class="text-muted mb-2">No saved e-signature found for Mayor.</div>
          <div class="mb-3">
            <label class="form-label">Upload your e-signature (.png only):</label>
            <input type="file" id="mayor-signature-upload" accept="image/png" class="form-control">
          </div>
        `;
      }
      ...
      const applyBtn = document.getElementById("apply-saved-signature");
      applyBtn.replaceWith(applyBtn.cloneNode(true)); // clear old event listener
      const newApplyBtn = document.getElementById("apply-saved-signature");

      newApplyBtn.addEventListener("click", async () => {
        // ✅ CASE 1: Mayor already has saved signature
        if (result.success && result.data?.signature_path) {
          modal.hide();
          await this.applyMayorSignature(result.data.signature_path);
          return;
        }

        // ✅ CASE 2: No saved signature — must upload first
        const fileInput = document.getElementById("mayor-signature-upload");
        if (!fileInput || !fileInput.files.length) {
          this.showError("Please select a PNG signature file to upload.");
          return;
        }
        ...
        try {
          ...
          const uploadResponse = await fetch("../api/admin/upload-signature.php", {
            method: "POST",
            body: formData,
            credentials: "include",
          });
          ...
          if (!uploadResult.success) {
            this.showError(uploadResult.message || "Signature upload failed.");
            return;
          }

          modal.hide();

          // ✅ Continue applying signature after upload
          await this.applyMayorSignature(uploadResult.signature_path);
        } catch (error) {
          console.error("Error uploading mayor signature:", error);
          this.showError("Network error during signature upload.");
        }
      });
    } catch (error) {
      console.error("Error loading mayor saved signature:", error);
      this.showError("Failed to load mayor saved signature.");
    }
  }

  // ✅ Helper: Apply the mayor's signature
  async applyMayorSignature(signaturePath) {
    try {
      const response = await this.adminAuth.makeAuthenticatedRequest("../api/admin/apply-signature.php", {
        method: "POST",
      ...
      if (res.success) {
        this.showSuccess("Mayor's e-signature applied successfully!");
        
        // ✅ Update signature image immediately
        const sigBox = document.getElementById("mayor-signature-box");
        if (sigBox) {
          sigBox.innerHTML = `<img src="../${signaturePath}" class="signature-image" alt="Mayor Signature">`;
        }
        
        // ✅ Update mayor name immediately
        if (res.data && res.data.full_name) {
          const mayorNameDisplay = document.getElementById("mayor-name-display");
          if (mayorNameDisplay) {
            mayorNameDisplay.textContent = res.data.full_name;
          }
        }
      } else {
        this.showError(res.message || "Failed to apply mayor's e-signature.");
      }
    } catch (error) {
      console.error("Error applying mayor signature:", error);
      this.showError("Network error while applying mayor's signature.");
    }
  }
  ...
  async updateApplicationStatus(applicationId, status) {
      ...
      // ✅ Check if mayor signature exists before approving for release
      if (status === "ready") {
          try {
              ...
              if (checkResult.success && !checkResult.data.mayor_signature_path) {
                  this.showError("You must sign the application form before approving for release");
                  return;
              }
          } catch (error) {
              console.error("Error checking mayor signature:", error);
              this.showError("Failed to verify mayor signature");
              return;
          }
      }

      // Determine confirmation message
      const confirmMessage = status === "ready"
          ? "Approve this application for release?"
          : "Reject this application?";

      // Get modal elements
      const statusModalEl = document.getElementById("statusConfirmModal");
      const statusMessageEl = document.getElementById("status-confirm-message");
      const confirmBtnEl = document.getElementById("confirm-status-btn");
      const statusModal = new bootstrap.Modal(statusModalEl);

      // Set modal message
      statusMessageEl.innerText = confirmMessage;
      ...
      newConfirmBtn.addEventListener("click", async () => {
          ...
          try {
              const response = await adminAuth.makeAuthenticatedRequest(
                  "../api/admin/update-status.php",
                  {
                      ...
                      body: JSON.stringify({
                          application_id: this.currentApplicationId,
                          status: status,
                          action: status === "ready" ? "approve_for_release" : "reject",
                      }),
                  }
              );
              ...
              if (result.success) {
                  ...
                  this.showSuccess(
                      status === "ready"
                          ? "Application approved for release successfully"
  ...
  toggleBatchMode() {
    ...
    if (this.batchMode) {
        ...
        toggleBtn.innerHTML = '<i class="bi bi-x-circle"></i> Cancel Batch'
        signSelectedBtn.classList.remove('d-none')
        selectColumns.forEach(col => col.style.display = '')
    } else {
        toggleBtn.classList.remove('active', 'btn-primary')
        toggleBtn.classList.add('btn-outline-primary')
        toggleBtn.innerHTML = '<i class="bi bi-check-square"></i> Batch Sign'
        signSelectedBtn.classList.add('d-none')
...
updateSelectedCount() {
    ...
    if (countSpan) {
        countSpan.textContent = this.selectedApplications.size
    }
    
    const signBtn = document.getElementById('sign-selected-btn')
    if (signBtn) {
        signBtn.disabled = this.selectedApplications.size === 0
    }
}
...
async batchSignApplications() {
    if (this.selectedApplications.size === 0) {
        this.showError('No applications selected')
        return
    }
    
    // Check for existing signature first
    const existingSignature = await this.checkExistingSignature()
    
    if (!existingSignature?.signature_path) {
        this.showError('Please upload your e-signature first')
        this.showSavedSignatureModal()
        return
    }
    
    // Show confirmation modal with updated message
    const confirmMessage = `Are you sure you want to sign ${this.selectedApplications.size} application(s)? 
    
This will apply your e-signature and mark them as Ready for Release.`
    ...
    statusMessageEl.innerHTML = confirmMessage.replace(/\n/g, '<br>')
    ...
    newConfirmBtn.addEventListener('click', async () => {
        statusModal.hide()
        await this.processBatchSigning(existingSignature.signature_path)
    })
    
    statusModal.show()
}

async processBatchSigning(signaturePath) {
    const applications = Array.from(this.selectedApplications)
    let successCount = 0
    let failCount = 0
    ...
    progressDiv.innerHTML = `
        <div class="d-flex align-items-center">
            ...
            <span>Processing applications... <span id="batch-progress">0/${applications.length}</span></span>
        </div>
    `
    document.body.appendChild(progressDiv)
    
    for (let i = 0; i < applications.length; i++) {
        const appId = applications[i]
        try {
            // Step 1: Apply signature
            const signResponse = await this.adminAuth.makeAuthenticatedRequest('../api/admin/apply-signature.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
            ...
            if (signResponse) {
                ...
                if (signResult.success) {
                    // Step 2: Update status to Ready for release
                    const statusResponse = await this.adminAuth.makeAuthenticatedRequest('../api/admin/update-status.php', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({
                            application_id: appId,
                    ...
                    if (statusResponse) {
                        ...
                        if (statusResult.success) {
                            ...
                            failCount++
                            console.error(`Failed to update status for ${appId}:`, statusResult.message)
                        }
                    } else {
                        failCount++
                    }
                    ...
                    failCount++
                    console.error(`Failed to sign application ${appId}:`, signResult.message)
                }
            } else {
                failCount++
            }
        ...
        // Update progress
        document.getElementById('batch-progress').textContent = `${i + 1}/${applications.length}`
    }
    ...
    // Show results
    if (successCount > 0) {
        this.showSuccess(`Successfully signed and approved ${successCount} application(s) for release${failCount > 0 ? `. ${failCount} failed` : ''}`)
    } else {
        this.showError(`Failed to process applications`)
    }
...
document.addEventListener("DOMContentLoaded", () => {
  cityMayorApplicants = new CityMayorApplicants()
  cityMayorApplicants.adminAuth = window.adminAuth
  cityMayorApplicants.bootstrap = window.bootstrap
})
