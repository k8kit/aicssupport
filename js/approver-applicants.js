// Approver applicants management
class ApproverApplicants {
  constructor() {
  ...
  async loadApplicants() {
    try {
      ...
      const response = await this.adminAuth.makeAuthenticatedRequest("../api/approver/applicants.php?" + queryString)
      if (!response) return
        ...
        <td>
          <div class="btn-group" role="group">
            <button class="btn btn-sm btn-outline-primary action-btn" 
              onclick="approverApplicants.viewApplication('${applicant.id}')"
              title="View Application">
              <i class="bi bi-file-text me-1"></i>
            </button>
            <button class="btn btn-sm btn-outline-danger action-btn ms-1" 
              onclick="approverApplicants.updateApplicationStatus('${applicant.id}', 'rejected')"
              title="Reject Application">
              <i class="bi bi-x-circle me-1"></i>
            </button>
            <button class="btn btn-sm btn-outline-success action-btn ms-1" 
              onclick="approverApplicants.updateApplicationStatus('${applicant.id}', 'waiting_mayor')"
              title="Forward to Mayor">
  ...
  setupModalEventListeners() {
    ...
    if (forwardBtn) {
      ...
      document.getElementById("forward-application").addEventListener("click", () => {
        this.updateApplicationStatus(this.currentApplicationId, "waiting_mayor")
      })
    }
  ...
  // ✅ Check if admin already has a saved e-signature
...
// ✅ Show Saved Signature Modal (with upload option if none)
  async showSavedSignatureModal() {
    try {
      ...
      // --- CASE 1: User already has saved signature ---
      if (result.success && result.data?.signature_path) {
        container.innerHTML = `
        ...
        container.innerHTML = `
          <div class="text-muted mb-2">No saved e-signature found.</div>
          <div class="mb-3">
            <label class="form-label">Upload your e-signature (.png only):</label>
            <input type="file" id="signature-upload" accept="image/png" class="form-control">
          </div>
        `;
      }
      ...
      newApplyBtn.addEventListener("click", async () => {
        // ✅ CASE 1: Has saved signature already
        if (result.success && result.data?.signature_path) {
          modal.hide();
          await this.applySignature(result.data.signature_path);
          return;
        }
        ...
        try {
          ...
          await this.applySignature(uploadResult.signature_path);
        } catch (error) {
  ...
  // ✅ Helper method: Apply the signature to current application
  async applySignature(signaturePath) {
    try {
      ...
      if (res.success) {
        ...
        const sigBox = document.getElementById("approver-signature-box");
        if (sigBox) {
          sigBox.innerHTML = `<img src="../${signaturePath}" class="signature-image" alt="Approver Signature">`;
        }
        
        // ✅ Update approver name immediately
        if (res.data && res.data.full_name) {
          const approverNameDisplay = document.getElementById("approver-name-display");
          if (approverNameDisplay) {
            approverNameDisplay.textContent = res.data.full_name;
          }
        }
      } else {
        this.showError(res.message || "Failed to apply e-signature.");
      }
    } catch (error) {
      console.error("Error applying signature:", error);
      this.showError("Network error while applying signature.");
    }
  }
  async updateApplicationStatus(applicationId, status) {
      ...
      // Get the confirmation modal and message elements
      ...
      // Determine confirmation message for forward
      if (status === "waiting_mayor") {
          // Check if approver signature exists before proceeding
          try {
              ...
              if (checkResult.success && !checkResult.data.approver_signature_path) {
                  this.showError("You must sign the application form before forwarding to City Mayor");
                  return;
              }
          } catch (error) {
              console.error("Error checking approver signature:", error);
              this.showError("Failed to verify approver signature");
              return;
          }

          statusMessageEl.innerText = "Forward this application to City Mayor for final approval?";
          ...
          newConfirmBtn.addEventListener("click", async () => {
              ...
              try {
                  ...
                  if (result.success) {
                      ...
                      const appModal = this.bootstrap.Modal.getInstance(document.getElementById("applicationModal"));
                      if (appModal) appModal.hide();

                      this.loadApplicants();
                      this.showSuccess("Application forwarded to City Mayor successfully.");
                  } else {
          ...
          statusModal.show();
          return; // ✅ Exit after showing forward confirmation
      }

      // ❌ REJECT FLOW — show rejection reason modal
      ...
      newConfirmRejectBtn.addEventListener("click", async () => {
          ...
          try {
              ...
              if (result.success) {
                  ...
                  this.loadApplicants();
                  this.showSuccess("Application rejected and email notification sent successfully.");
              } else {
  ...
  getStatusClass(status) {
    const statusMap = {
      ...
      Approved: "approved",
      Rejected: "rejected",
      "Waiting for approval of heads": "waiting_mayor",
...
document.addEventListener("DOMContentLoaded", () => {
  approverApplicants = new ApproverApplicants()
  approverApplicants.adminAuth = window.adminAuth // Assign adminAuth variable
  approverApplicants.bootstrap = window.bootstrap // Assign bootstrap variable
})
